defmodule MyApp.MonitoringTools.MemoryWatcher do
  use GenServer
  ...
  def handle_info(:log_top_n, state) do
    reduction_hogs =
      Process.list()
      |> Enum.map(fn pid ->
        {:reductions, measurement} = Process.info(pid, :reductions)
        {pid, measurement}
      end)
      |> Enum.sort_by(fn {_, total_heap_size} -> total_heap_size end, :desc)
      |> Enum.take(@top_n)

    Logger.info("Top reduction processes: #{inspect(reduction_hogs)}")
